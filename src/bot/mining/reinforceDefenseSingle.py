...
def reinforceDefenseSingle(user: User, currentTeamId: int) -> int:
    """
    ...
    reinforceableMines = [m for m in fetchOpenMines(
        user) if minerCanReinforce(m)]
    ...
    for mine in reinforceableMines:
        ...
        if int(currentTeamId) == int(mine["team_id"]):
            try:
